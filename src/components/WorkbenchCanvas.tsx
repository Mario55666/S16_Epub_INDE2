...
import {
  MiniTextInput, MiniEmail, MiniCheckbox, MiniRadioButton,
  MiniDropdown, MiniTextarea, MiniSubmitButton,
  MiniVideo, MiniAudio, MiniTabs, MiniAccordion,
  MiniTooltip, MiniModal, MiniCarousel,
  MiniProgressBar, MiniHoverCard, MiniScrollReveal,
} from './InteractiveMiniPreviews';
import { X, MousePointer } from 'lucide-react';

const miniPreviewMap: Record<string, React.FC> = {
  'text-input': MiniTextInput,
  'email-input': MiniEmail,
  'checkbox': MiniCheckbox,
  'radio-button': MiniRadioButton,
  'dropdown': MiniDropdown,
  'textarea': MiniTextarea,
  'submit-button': MiniSubmitButton,
  'video': MiniVideo,
  'audio': MiniAudio,
  'tabs': MiniTabs,
  ...
  'progress-bar': MiniProgressBar,
  'hover-card': MiniHoverCard,
  'scroll-reveal': MiniScrollReveal,
};
          ...
          {droppedItems.map(item => {
            ...
            return (
              ...
              >
                ...
                <div className="mt-1 pointer-events-auto" onMouseDown={e => e.stopPropagation()}>
                  {miniPreviewMap[comp.id] ? (
                    (() => {
                      const MiniComp = miniPreviewMap[comp.id];
                      return <MiniComp />;
                    })()
                  ) : (
                    <>
                      <div className="flex items-center gap-1">
                        <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: layerColor }} />
                        <span className="font-medium truncate text-[9px]">{comp.name}</span>
                      </div>
                      <div className="text-[7px] text-muted-foreground/60 mt-0.5 truncate">
                        {comp.categoryLabel}
                      </div>
                    </>
                  )}
                ...
                {isSelected && (
                  <div className="text-[7px] text-primary/70 font-mono mt-1 pointer-events-none">
